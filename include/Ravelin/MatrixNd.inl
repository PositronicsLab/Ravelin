 ...
 */
template <class M>
M& MatrixN::get_sub_mat(unsigned row_start, unsigned row_end, unsigned col_start, unsigned col_end, M& m, MatrixN::Transposition trans) const
{
  if (row_start > row_end || row_end > _rows || col_start > col_end || col_end > _columns)
    throw InvalidIndexException();

  // resize the matrix
  if (trans == MatrixN::eNoTranspose)
 ...
 */
template <class M>
MatrixN& MatrixN::set_sub_mat(unsigned row_start, unsigned col_start, const M& m, MatrixN::Transposition trans)
{
  if (trans == MatrixN::eNoTranspose)
  {
