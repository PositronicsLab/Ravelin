...
/// Constructs a real-valued attribute with the given name
XMLAttrib::XMLAttrib(const std::string& name, float real_value)
{
  this->name = name;
  this->value = str(real_value);
  this->processed = false;
...
/// Constructs a Origin3d-valued attribute with the given name
XMLAttrib::XMLAttrib(const std::string& name, const Origin3f& o)
{
  this->name = name;
  std::ostringstream oss;
  oss << o[0] << " " << o[1] << " " << o[2];
...
/// Constructs a Vector2d-valued attribute with the given name
XMLAttrib::XMLAttrib(const std::string& name, const Vector2f& v)
{
  this->name = name;
  std::ostringstream oss;
  oss << v[0] << " " << v[1];
...
/// Constructs a Vector3d-valued attribute with the given name
XMLAttrib::XMLAttrib(const std::string& name, const Vector3f& v)
{
  this->name = name;
  std::ostringstream oss;
  oss << v[0] << " " << v[1] << " " << v[2];
...
/// Constructs a Quatd-valued attribute with the given name
XMLAttrib::XMLAttrib(const std::string& name, const Quatf& q)
{
  this->name = name;
  std::ostringstream oss;
  oss << q.w << " " << q.x << " " << q.y << " " << q.z;
...
/// Constructs a vector-valued attribute with the given name
XMLAttrib::XMLAttrib(const std::string& name, const VectorNf& vector_value)
{
  this->processed = false;
  this->name = name;
  std::ostringstream oss;
...
/// Constructs a vector-valued attribute with the given name
XMLAttrib::XMLAttrib(const std::string& name, const SVector6f& vector_value)
{
  this->processed = false;
  this->name = name;
  std::ostringstream oss;
...
/// Constructs a vector-valued attribute with the given name
XMLAttrib::XMLAttrib(const std::string& name, const MatrixNf& matrix_value)
{  
  this->processed = false;
  this->name = name;
  std::ostringstream oss;
...
/// Constructs a vector-valued attribute with the given name
XMLAttrib::XMLAttrib(const std::string& name, const Matrix3f& matrix_value)
{  
  this->processed = false;
  this->name = name;
  std::ostringstream oss;
...
/// Gets a real value as a string
std::string XMLAttrib::str(float value)
{
  if (value == std::numeric_limits<double>::infinity())
    return std::string("inf");
  else if (value == -std::numeric_limits<double>::infinity())
...
void XMLAttrib::get_real_value(double& value)
{
  ...
  if (strcasecmp(this->value.c_str(), "inf") == 0)
    ...
    value = (double) std::atof(this->value.c_str());
}

/// Gets a floating point value from the underlying string representation
void XMLAttrib::get_real_value(float& value)
{
  // indicate this attribute has been processed
  processed = true;
  ...
  if (strcasecmp(this->value.c_str(), "inf") == 0)
    ...
    value = (float) std::atof(this->value.c_str());
}
...
shared_ptr<const XMLTree> XMLTree::construct_xml_tree(xmlNode* root)
{
  ...
  shared_ptr<XMLTree> node(new XMLTree(std::string((char*) root->name)));
...
/// Gets a list of space-delimited and/or comma-delimited vectors from the underlying string value
void XMLAttrib::get_vector_value(VectorNf& v)
{
  // indicate this attribute has been processed
  processed = true;

  v = VectorNf::parse(value);
}

/// Returns an Origin3d value from the attribute
void XMLAttrib::get_origin_value(Origin3d& o) 
{
  // indicate this attribute has been processed
  processed = true;
...
/// Returns an Origin3d value from the attribute
void XMLAttrib::get_origin_value(Origin3f& o) 
{
  // indicate this attribute has been processed
  processed = true;

  VectorNf v = VectorNf::parse(value);
  if (v.size() != 3)
    throw std::runtime_error("Unable to parse origin from vector!");
  o.x() = v[0];
  o.y() = v[1];
...
void XMLAttrib::get_quat_value(Quatd& q)
{
  // indicate this attribute has been processed
  processed = true;
...
/// Returns a quaternion value from the attribute
void XMLAttrib::get_quat_value(Quatf& q)
{
  // indicate this attribute has been processed
  processed = true;
...
void XMLAttrib::get_rpy_value(Quatd& q)
{
  // indicate this attribute has been processed
  processed = true;
  ...
  q = Quatd::rpy(v[0], v[1], v[2]);
}

/// Returns a quaternion value from a roll-pitch-yaw attribute
void XMLAttrib::get_rpy_value(Quatf& q)
{
  // indicate this attribute has been processed
  processed = true;
...
/// Gets a list of space-delimited and/or comma-delimited vectors from the underlying string value
void XMLAttrib::get_vector_value(Vector2f& v)
{
  // indicate this attribute has been processed
  processed = true;

  VectorNf w = VectorNf::parse(value);
  if (w.size() != v.size())
    throw MissizeException();
  v = Vector2f(w[0], w[1]);
}  
...
/// Gets a list of space-delimited and/or comma-delimited vectors from the underlying string value
void XMLAttrib::get_vector_value(Vector3f& v)
{
  // indicate this attribute has been processed
  processed = true;
...
/// Gets a list of space-delimited and/or comma-delimited vectors from the underlying string value
void XMLAttrib::get_vector_value(SVector6f& v)
{
  // indicate this attribute has been processed
  processed = true;
  ...
  if (w.size() != v.size())
    throw MissizeException();
  v = SVector6f(w[0], w[1], w[2], w[3], w[4], w[5]);
}  
...
/// Gets a list of space-delimited and/or comma-delimited strings from the underlying string value
void XMLAttrib::get_matrix_value(Matrix3f& m)
{
  // indicate this attribute has been processed
  processed = true;
...
/// Gets a list of space-delimited and/or comma-delimited strings from the underlying string value
void XMLAttrib::get_matrix_value(MatrixNf& m)
{
  // indicate this attribute has been processed
  processed = true;
...
std::ostream& Ravelin::operator<<(std::ostream& out, const XMLAttrib& attr)
{
...
std::list<shared_ptr<const XMLTree> > XMLTree::find_child_nodes(const std::list<std::string>& names) const
{
  ...
  for (std::list<shared_ptr<XMLTree> >::const_iterator i = this->children.begin(); i != this->children.end(); i++)
  {
...
std::ostream& Ravelin::operator<<(std::ostream& out, const XMLTree& node)
{
  // get the list of child nodes
  const std::list<shared_ptr<XMLTree> >& child_nodes = node.children; 
  ...
  else
  {
    ...
    for (std::list<shared_ptr<XMLTree> >::const_iterator i = child_nodes.begin(); i != child_nodes.end(); i++)
      out << **i;
